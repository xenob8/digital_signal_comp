...
clc;
clear;
sp=75;%кол-во отсчетов за период несущей частоты радиоимпульса
...
for shum = [0, 0.05, 0.1, 0.15, 0.2]
    for i4=1:2048
    shift=400;% точное значение временной задержки     
    for i=1:kt
        y(i)=0;
        sy(i)=0;
        w(i)=0;
    ...
    %вычисление СКО и коэффициента корреляции X и Y
                for i=1:kt
                   ...
                   % kor1(k)=kor1(k)+sxy(i);%вычисление нормы Минковского/Поддорогина !
                end 
