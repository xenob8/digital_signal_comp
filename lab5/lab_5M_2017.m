...
Q=0.2;%1уровень шума в долях СКО

clc;%очистка Command Window
for shift = [300, 310, 320,330,340, 350]
    ...
    %вычисление СКО и коэффициента корреляции X и Y
              ...
              % sxy(i)=(abs(x(i)+y(i))); %вычисление нормы Поддорогина
                   %kor1(k)=kor1(k)+sxy(i);%вычисление нормы Минковского и Поддорогина
                end 
    ...
    err_ma=abs(dt_ma-shift)/shift*100;
    dt_acm=dt;% %оценка времени задержки по методу АКМ
    err_acm= abs(dt-shift)/shift*100;
    
    table(shift,dt_ma,err_ma,dt_acm,err_acm )
end
pause;
